...
const inputMovieSearchEl = document.getElementById("input-movie-search");
const apikey = "3475c7ce";
...
async function fetchSearchAPI() {
  let movieName = inputMovieSearchEl.value.trim();
...
async function fetchSearchMovieID() {
  const movies = await fetchSearchAPI();
...
async function renderHTML() {
  const movieDetailsArray = await fetchSearchMovieID();
  let html = "";
  for (const movieDetails of movieDetailsArray) {
    const { Title, imdbRating, Runtime, Genre, Poster, Plot } = movieDetails;

    const movieHtml = `<div class="poster">
...
<p class="movie-plot">${Plot}</p>
  ...
  filmContainerEl.innerHTML = html;
}
